...
Function sizing_integrate_uas_cloudy(inst,tb,isFilter)
	string inst
	variable tb
	variable isFilter

	integrate_NSVM_fraction(inst,tb,"msems",0,isFilter)
	integrate_NSVM_fraction(inst,tb,"pops",0,isFilter)

End

Function sizing_integrate_uas_cloudy_2d(inst,tb,isFilter,isAmbient,shape)
	string inst
	variable tb
	variable isFilter
	variable isAmbient
	...
	integrate_NSVM_fraction_2d(inst,tb,"msems",0,isFilter,isAmbient,shape)
	integrate_NSVM_fraction_2d(inst,tb,"pops",0,isFilter,isAmbient,shape)
...
Function integrate_NSVM_fraction(inst,tb,fraction,diam_type,isFilter)
	...
	for (i=0; i<rows; i+=1)
		...
		for (j=0; j<cols; j+=1)
			...
			elseif ( cmpstr(fraction,"msems")==0 ) // .010 < Dp <= .312um
//				if (i>42)
//					print "here"
//				endif
				if ( (dpa[j] > 0.010) && (dpa[j] <= 0.312) )
					intN[i] += dN[i][j]
					intS[i] += dS[i][j]
					intV[i] += dV[i][j]
					intM[i] += dM[i][j]
				endif
			elseif ( cmpstr(fraction,"pops")==0 ) // .133um < Dp <= 3.361um
				if ( (dpa[j] > 0.133) && (dpa[j] <= 3.361) )
					intN[i] += dN[i][j]
					intS[i] += dS[i][j]
					intV[i] += dV[i][j]
					intM[i] += dM[i][j]
				endif
			elseif ( cmpstr(fraction,"aitken")==0 ) // .010 < Dp <= .07um
//				if (i>42)
//					print "here"
//				endif
				if ( (dpa[j] > 0.010) && (dpa[j] <= 0.07) )
					intN[i] += dN[i][j]
					intS[i] += dS[i][j]
					intV[i] += dV[i][j]
					intM[i] += dM[i][j]
				endif
			elseif ( cmpstr(fraction,"accum")==0 ) // .07 um < Dp <= 0.8 um
				if ( (dpa[j] > 0.07) && (dpa[j] <= .8) )
					intN[i] += dN[i][j]
					intS[i] += dS[i][j]
					intV[i] += dV[i][j]
					intM[i] += dM[i][j]
...
//	SetScale/P x dt[0],tb,"dat", intM
	...
	for (i=0; i<rows; i+=1)
		...
		for (j=0; j<cols; j+=1)
			...
			elseif ( cmpstr(fraction,"msems")==0 ) // .010 < Dp <= .312um
				if ( (dpa[i][j] > 0.010) && (dpa[i][j] <= 0.311) )
					intN[i] += dN[i][j]
					intS[i] += dS[i][j]
					intV[i] += dV[i][j]
					intM[i] += dM[i][j]
				endif
			elseif ( cmpstr(fraction,"pops")==0 ) // .122um < Dp <= 3.361um
				if ( (dpa[i][j] > 0.122) && (dpa[i][j] <= 3.361) )
					intN[i] += dN[i][j]
					intS[i] += dS[i][j]
					intV[i] += dV[i][j]
					intM[i] += dM[i][j]
...
Function dmps_toggle_nans_from_marquee(allPoints,isNaN)
	...
	string dw_name = ""
	string dpname = ""
	for (row=0; row<dimsize(wlist,0); row+=1)
		if (strsearch(wlist[row][0], "dlogDp", 0) >= 0)
			dw_name = wlist[row][1]
		elseif (strsearch(wlist[row][0], "_dp", 0) >= 0)
			dpname = wlist[row][1]
		endif
	endfor
...
//	wave t = $(wlist[1][1])     // datetime wave
//	variable idx = strsearch(wlist[2][1],"_",Inf,1)
	variable idx = strsearch(dpname,"_",Inf,1)
//	string dpname = wlist[2][1]
...
Function/S sizing_goto_sizing()
	string inst
	
	string sdf = getdatafolder(1)
	setdatafolder root:
	newdatafolder/o/s sizing
	
	return sdf
End

Function/S sizing_goto_inst(inst)
	string inst
	
	string sdf = sizing_goto_sizing()
	newdatafolder/o/s $inst

	return sdf
End

Function sizing_concatenate_legs(tb, leg_list, [inst_list, use_2d])
	variable tb
	string leg_list
	string inst_list
	variable use_2d
	...
	if (ParamIsDefault(inst_list))
		inst_list = "dmps;aps"
	endif
	
	if (ParamIsDefault(use_2d))
		use_2d = 0
	endif
...
//	endif
	...
	for (insti=0; insti<itemsinlist(inst_list); insti+=1)
		...
		variable legi
		for (legi=0; legi<itemsinlist(leg_list); legi+=1)
			sizing_goto_sizing()
			setdatafolder $(":"+stringfromlist(legi, leg_list)+":"+inst+":data_"+num2str(tb)+"sec:")
			wave dt = $(inst+"_datetime")
			wavestats/Q dt
			if (legi==0)		
				mintime = V_min
...
//	endfor
		...
		sizing_goto_inst(inst)
		newdatafolder/o/s $("data_"+num2str(tb)+"sec")
		make/o/n=(time_periods)/d $(inst+"_datetime")
		wave inst_dt = $(inst+"_datetime")

		make/o/n=(time_periods+1)/d $(inst+"_datetime_im")
		wave inst_dt_im = $(inst+"_datetime_im")
		
		inst_dt[0] = mintime
		inst_dt_im[0] = mintime
		variable i
		for (i=0; i<time_periods; i+=1)
			inst_dt[i] = inst_dt[i-1]+tb
			inst_dt_im[i] = inst_dt_im[i-1]+tb
		endfor

		inst_dt_im[numpnts(inst_dt_im)-1] = inst_dt[numpnts(inst_dt)-1] + tb
		
		string dn_list = "dNdlogDp;dSdlogDp;dVdlogDp;dN;dS;dV;intN;intS;intV;density_w;user_nan;dNdlogDp_raw"
		for (legi=0; legi<itemsinlist(leg_list); legi+=1)
			sizing_goto_sizing()
			setdatafolder $(":"+stringfromlist(legi, leg_list)+":"+inst+":data_"+num2str(tb)+"sec:")
			
			// if first time through, copy non-time waves and create time-based waves
			if (legi==0)
				string dp_list = "dp;dpa;dpa_im;dpa_um;dpa_um_im;dpva;dpva_im;dpva_um;dpva_um_im;dp_im;dp_native;dp_um;dp_um_im;dlogDp"
				make/WAVE/o/n=(itemsinlist(dp_list)) dp_wave_list
				wave/WAVE dp_waves = dp_wave_list
				variable listi
				for (listi=0; listi<itemsinlist(dp_list); listi+=1)
					string wname = inst+"_"+stringfromlist(listi, dp_list)
					dp_waves[listi] = $wname
				endfor
				
				string leg_df = sizing_goto_inst(inst)
				setdatafolder $("data_"+num2str(tb)+"sec")		
				for (listi=0; listi<itemsinlist(dp_list); listi+=1)
					wname = inst+"_"+stringfromlist(listi, dp_list)
					duplicate/o dp_waves[listi], $wname
				endfor
				killwaves/Z dp_waves
				
				setdatafolder leg_df
				make/WAVE/o/n=(itemsinlist(dn_list)) dn_wave_list
				wave/WAVE dn_waves = dn_wave_list
				wave leg_dt = $(inst+"_datetime")
				for (listi=0; listi<itemsinlist(dn_list); listi+=1)
					wname = inst+"_"+stringfromlist(listi, dn_list)
					dn_waves[listi] = $wname
				endfor

				leg_df = sizing_goto_inst(inst)
				setdatafolder $("data_"+num2str(tb)+"sec")		
				for (listi=0; listi<itemsinlist(dn_list); listi+=1)
					wname = inst+"_"+stringfromlist(listi, dn_list)
					wave w = dn_waves[listi]
					if (dimsize(w, 1) > 0) 
						make/o/n=(numpnts(inst_dt), dimsize(w, 1)) $wname
						wave par = $wname
						par = NaN
						SetScale/P x inst_dt[0],tb,"dat", par

						variable starti = x2pnt(par,leg_dt[0])
						par[starti,starti+numpnts(w)] = w[p-starti][q]
										
					else
					
						make/o/n=(numpnts(inst_dt)) $wname
						wave par = $wname
						par = NaN
						SetScale/P x inst_dt[0],tb,"dat", par

						starti = x2pnt(par,leg_dt[0])
						par[starti,starti+numpnts(w)] = w[p-starti]
					
					endif						
						
				endfor
				killwaves/Z dn_waves
...
//	fprintf fd, "NAAMES3: size distribution (" + id_str +") data - v" + version_str
	fprintf fd, "ATOMIC: size distribution (" + id_str +") data - v" + version_str
	variable i,j
...
//	NewPath/O/Q save_path_filter, "Y:Data:Cruises:naames:Leg4:sizing:dmps:final_data"
	NewPath/O/Q save_path, "Y:Data:Cruises:atomic:sizing:final_data"
	NewPath/O/Q save_path_filter, "Y:Data:Cruises:atomic:sizing:final_data"
...
if (1)	
	...
	// add lat, lon to file
	wave lat = root:cn:data_300sec:Latitude
	wave lon =root:cn:data_300sec:Longitude
...
Function sizing_par_to_sizing_tb(tb, par_name)
	variable tb
	string par_name
	
	wave par = $par_name
	newdatafolder/o $("data_"+num2str(tb)+"sec")
	
	wave dt = root:sizing:dmps:data_300sec:dmps_datetime
	make/o/n=(numpnts(dt)) $(":data_"+num2str(tb)+"sec:"+par_name)
	wave par_tb = $(":data_"+num2str(tb)+"sec:"+par_name)
	
	variable i
	for (i=0; i<numpnts(dt); i+=1)
		variable index = x2pnt(par, dt[i])
		par_tb[i] = par[index]
	endfor
