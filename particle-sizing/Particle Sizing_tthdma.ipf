...
Menu "ACG Data"
	submenu "Particle Sizing"
		submenu "SMPS"
			submenu "ATOMIC..."
				"load ThSMPS", load_smps_rows(tb=120)
				"load MART", load_smps_rows(tb=300)
			end
			submenu "Load data..."
				"from rows", load_smps_rows()
			end
			"Load valve data", ttdma_load_valve()
...
//	SetScale/P x dt[0],tb,"dat", intM
	...
	for (i=0; i<rows; i+=1)
		...
		for (j=0; j<cols; j+=1)
			...
			elseif ( cmpstr(fraction,"gt20nm")==0 ) // 0.1um < Dp < 0.7um
				if ( (dpa[i][j] >= 0.2) )
					intN[i] += dN[i][j]
					intS[i] += dS[i][j]
					intV[i] += dV[i][j]
...
Function smps_toggle_nans_from_marquee(allPoints,isNaN)
	...
	string dw_name = "root:smps:data_"+num2str(smps_defaultTimeBase)+"sec:smps_dNdlogDp"
	string dp_name = "root:smps:data_"+num2str(smps_defaultTimeBase)+"sec:smps_dp_um_im"
	string dt_name = "root:smps:data_"+num2str(smps_defaultTimeBase)+"sec:smps_datetime_im"
...
//	SetScale/P x valve_dt[0],1,"dat", valve 
	...
	for (i=0; i<numpnts(dt); i+=1)
		...
		// fix bad plumibing times
		if ( (dt[i] >= start_bad_atomic_dt && dt[i] <= start_bad_dl_atomic_dt) || (dt[i] >= stop_bad_dl_atomic_dt && dt[i] <= stop_bad_atomic_dt))
...
Function ttdma_fake_valve(starti, stopi)
	variable starti
	variable stopi
...
//	for (row=0; row<numpnts(dt); row+=1)
	for (row=starti; row<=stopi; row+=1)
